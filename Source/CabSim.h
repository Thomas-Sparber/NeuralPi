...
/**
    Used by the CabSim to dispatch engine-update messages on a background
    thread.

    May be shared between multiple CabSim instances.

    @tags{DSP}
*/
class JUCE_API CabSimMessageQueue
{
public:
    /** Initialises the queue to a default size.

        If your CabSim is updated very frequently, or you are sharing
        this queue between multiple CabSims, consider using the alternative
        constructor taking an explicit size argument.
    */
    CabSimMessageQueue();
    ~CabSimMessageQueue() noexcept;
    ...
    /** Initialises the queue with the specified number of entries.
        ...
        of CabSims sharing the same Queue, and the frequency of updates
        to those CabSims.
    */
    explicit CabSimMessageQueue (int numEntries);

    CabSimMessageQueue (CabSimMessageQueue&&) noexcept;
    CabSimMessageQueue& operator= (CabSimMessageQueue&&) noexcept;

    CabSimMessageQueue (const CabSimMessageQueue&) = delete;
    CabSimMessageQueue& operator= (const CabSimMessageQueue&) = delete;
...
/**
    Performs stereo partitioned CabSim of an input signal with an
    ...
    CabSim done in the FIRFilter class, with a FIRFilter::Coefficients
    ...
    CabSim when the size of the impulse response is 64 samples or
    greater.
    ...
    @tags{DSP}
*/
class JUCE_API  CabSim
{
public:
    //==============================================================================
    /** Initialises an object for performing CabSim in the frequency domain. */
    CabSim();

    /** Initialises a CabSim engine using a shared background message queue.

        IMPORTANT: the queue *must* remain alive throughout the lifetime of the
        CabSim.
    */
    explicit CabSim (CabSimMessageQueue& queue);

    /** Contains configuration information for a CabSim with a fixed latency. */
    struct Latency { int latencyInSamples; };

    /** Initialises an object for performing CabSim with a fixed latency.
        ...
        non-zero latency can reduce the CPU consumption of the CabSim
        algorithm.

        @param requiredLatency        the minimum latency
    */
    explicit CabSim (const Latency& requiredLatency);

    /** Contains configuration information for a non-uniform CabSim. */
    ...
    /** Initialises an object for performing CabSim in the frequency domain
        ...
        @param requiredHeadSize       the head IR size for two stage non-uniform
                                      partitioned CabSim
     */
    explicit CabSim (const NonUniform& requiredHeadSize);
    ...
    /** Behaves the same as the constructor taking a single Latency argument,
        ...
        CabSim.
    */
    CabSim (const Latency&, CabSimMessageQueue&);
    ...
    /** Behaves the same as the constructor taking a single NonUniform argument,
        ...
        CabSim.
    */
    CabSim (const NonUniform&, CabSimMessageQueue&);
    ...
    */
    ...
    /** This function loads an impulse response audio file from memory, added in a
        ...
        data remains constant throughout the lifetime of the CabSim object, as
    ...
    */
    ...
    /** This function returns the current latency of the process in samples.
        ...
        Note: This is the latency of the CabSim engine, not the latency
...
private:
    ...
    CabSim (const Latency&,
                 const NonUniform&,
                 OptionalScopedPointer<CabSimMessageQueue>&&);

    void processSamples (const dsp::AudioBlock<const float>&, dsp::AudioBlock<float>&, bool isBypassed) noexcept;
    ...
    public:
        ...
        void processSamples (const dsp::AudioBlock<const float>&,
